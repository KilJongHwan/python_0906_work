...
# 저장은 Rest API의 기본형태인 JSON으로 저장 및 불러오기 (웹에서 API의 기본포멧)
# 힘수로 구현
...
# 학생 정보 변경
def modify_student():
    student_id = input("수정할 학번을 입력 : ")
    student_info = student_map.get(student_id) # 학번을 키로 해서 해당하는 값(이름과 주소로 구분된 딕셔너리)
    if student_info:
...
def del_student():
    ...
    if student_map.get(student_id):
        del student_map[student_id] # key로 헤당 딕셔너리 삭제
        print("학생 정보가 삭제 되었습니다.")
    else: print("해당 학번의 학생을 찾을 수가 없습니다.")
...
def load_student():
    try :
        ...
        print("학생 정보를 불러 왔습니다.")
    except FileExistsError:
        print("학생 정보를 불러올수가 없습니다.")
while True :
