...
# 추상 메소드가 포함된 부모 클래스는 객체로 만들수 없고 단지 상속을 주기 위해서 존재
from abc import *

class NetworkAdapter(metaclass=ABCMeta):  # 추상 클래스 선언
    @abstractmethod
    def connect(self): pass # 구현할 내용이 없는 경우에 pass 키워드를 사용

class FiveG(NetworkAdapter):
    def __init__(self, company):  # 생성자 만들기
        self.company = company

    def connect(self):  # 부모에게서 상속 받은 추상 메소드 구현
...
if net == "1":
    adapter = WiFi("KT Megepass")
    adapter.connect()
elif net == "2" :
    adapter = FiveG("SKT broadband")
    adapter.connect()
