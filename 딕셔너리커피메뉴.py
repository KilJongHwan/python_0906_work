menu_dic = {
    # 메뉴이름 [카테고리, 가격 ,설명]
    "Americano": ["Coffee", 2000, "기본 커피 입니다."],    # key와 value으로 구성되며, 값을 리스트로 만들어 여러가지 정보를 포함시킴
...
while True:
    print("메뉴를 선택하세요")
    menu = input("[1]메뉴 리스트 보기 [2]메뉴 조회 [3]메뉴 추가 [4]메뉴 삭제 [5]종료 : ") # input 기본 문자열로 반환 딤
    if menu == "1" :
        for key in menu_dic:
        ...
        if add_menu not in menu_dic:
            category = input("카테고리 입결 : ")
            price = int(input("가격 입력 : "))
            desc = input("제품 설명 입력 : ")
            menu_dic[add_menu] = [category,price,desc] # 추가 : 딕셔너리[키] = 값 새로운 키와 값을 추가
        else :
            print("메뉴가 이미 존재 합니다.")
        for key in menu_dic :   # 메뉴가 추가되면 업데이트된 메뉴 리스트를 다시 출력
            print(key, ":", menu_dic[key])
    elif menu == "4":
        ...
        if del_menu in menu_dic :
            ...
            print("삭제할 메뉴가 없습니다.")
        for key in menu_dic:  # 메뉴가 삭제되면 업데이트된 메뉴 리스트를 다시 출력
            print(key, ":", menu_dic[key])
    elif menu == "5":
        print("메뉴를 종료합니다.")
        break
