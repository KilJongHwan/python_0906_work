...
# 입금 함수
def deposit(input):  # 잔고와 입금액을 매개변수로 전달빋음
    global balance  # global 변수
    balance += input
...
def withdraw(output):
    ...
    if balance >= output:
        ...
        print(f"출금에 실패 했습니다. 잔액은 {balance}입니다.")


balance = 0  # 외부에서 선언했지만 메게뱐수로 잔달하여 결과를 리턴받음
name = open_account("james")
deposit(1000)
withdraw(500)
